...
import numpy as np
import models.configs_FocusMorph as configs
import math
from einops import rearrange
import nibabel as nib


def save_flow(flow, file_name):
    # 保存flow到results文件夹
    ...
    # 创建一个Nifti1Image对象
    nii_img = nib.Nifti1Image(flow, affine=np.eye(4))  # 如果您有仿射矩阵，请将其传递给affine参数

    # 保存为.nii.gz文件
    nib.save(nii_img, 'flow_results/{}.nii.gz'.format(file_name))
...
class FocusedLinearAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        num = int(math.ceil(v.shape[1] ** (1/3)))
        feature_map = rearrange(v, "b (d w h) c -> b c d w h", w=num, h=num)
        feature_map = rearrange(self.dwc(feature_map), "b c d w h -> b (d w h) c")
        x = x + feature_map
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, attn_type, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, attn_type='FocusedLinearAttention',
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer( attn_type=self.attn_type,
...
class MultilayerEncoder(nn.Module):
    def __init__(self, in_channels, out_channels, kernal_size=3, stride=1, padding=1, alpha=0.1):
        ...
        self.conv = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, kernal_size, 1, 1),
            nn.Conv3d(out_channels, out_channels, kernal_size, 1, 1),
            nn.Conv3d(out_channels, out_channels, kernal_size, 1, 1),
            nn.LeakyReLU(alpha),
...
class ShallowEncoder(nn.Module):
    def __init__(self, dim):
        ...
        # 浅层特征提取
        self.shallow = nn.Conv3d(1, dim, kernel_size=3, stride=1, padding=1)

        # 3D深度卷积
        self.conv0 = nn.Conv3d(3, dim, (5, 5, 5), padding=(2, 2, 2), groups=dim)
        # 3D深度空洞卷积
        self.conv_spatial = nn.Conv3d(
            dim, dim, (7, 7, 7), stride=1, padding=(9, 9, 9), groups=dim, dilation=(3, 3, 3))
        # 3D逐点卷积
        self.conv1 = nn.Conv3d(dim, dim, (1, 1, 1))
...
class MotionFusion(nn.Module):
    def __init__(self, in_channels, out_channels, kernal_size=3, stride=1, padding=1, alpha=0.1):
        ...
        self.bias = nn.Parameter(torch.zeros(1, 1, 1, 1, 1))

    def forward(self, fix, flow):
        res_flow = flow
        fix_x = fix[:, 0:1, :, :, :]
        fix_y = fix[:, 1:2, :, :, :]
        fix_z = fix[:, 2:3, :, :, :]
        ...
        out_x = fix_x * self.weight + flow_x * (1 - self.weight) + self.bias
        out_y = fix_y * self.weight + flow_y * (1 - self.weight) + self.bias
        out_z = fix_z * self.weight + flow_z * (1 - self.weight) + self.bias

        flow = torch.cat([out_x, out_y, out_z], dim=1)

        return res_flow + flow
...
class FocusMorph(nn.Module):
    def __init__(self, config):
        '''
        TransMorph Model
        '''
        super(FocusMorph, self).__init__()
        ...
        self.transformer = SwinTransformer(attn_type=self.attn_type,
        ...
        self.reg_head0 = RegistrationHead(
            in_channels=192 * 2,
            out_channels=3,
            kernel_size=3,
        )
        ...
        # 定义浅层特征提取
        self.shallow_encoder = LKA(3)

        self.multilayer_encoder = MultilayerEncoder(3,3)

        self.motion_fusion = MotionFusion(3, 3)

    def forward(self, x):
        source = x[:, 0:1, :, :]

        fix = x[:, 1:2, :, :]
        fix = self.shallow_encoder(fix)
        ...
        fix_outs = self.multilayer_encoder(fix)

        x = self.up0(out_feats[-1], f1)
        flow0 = self.reg_head0(x)
        flow0 = self.motion_fusion(fix_outs[-1], flow0)
        flow0 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False)(flow0)


        x = self.up1(x, f2)
        flow1 = self.reg_head1(x)
        flow1 = self.motion_fusion(fix_outs[-2], flow1 + flow0)
        flow1 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False)(flow1)

        x = self.up2(x, f3)
        flow2 = self.reg_head2(x)
        flow2 = self.motion_fusion(fix_outs[-3], flow2 + flow1)
        flow2 = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False)(flow2)

        x = self.up3(x, f4)
        flow3 = self.reg_head3(x)
        flow3 = self.motion_fusion(fix_outs[-4], flow3 + flow2)

        flow = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False)(flow3)
...
CONFIGS = {
    'FocusMorph': configs.get_3DTransMorph_config(),
    'FocusMorph-Small': configs.get_3DTransMorphSmall_config(),
}
